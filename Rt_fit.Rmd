...
dat <- dat_all$ODE[, c("time", "daily_infected", "daily_onset", "daily_confirmed", "Rt")]
names(dat) <- c("t", "daily_infected", "daily_symptom_onset", "daily_confirmed", "Rt")

## run particle filter
devtools::load_all(".")
theta["betavol"] <- 0.40
y0 <- c(S = 1e7, E = 0, P = 0, I = 10, R = 0, CE = 0, CI = 10, CR = 0)
...
sample_ids <- sample(1:100, 10, replace = FALSE)
data_type <- "infection"
...
var_names <- function(data_type = c("infection", "symptom onset", "confirmation")) {
  type <- match.arg(data_type)
  model_var <- "CE"
  dat_var <- "daily_infected"
  
  if (type == "symptom onset") {
    model_var <- "CI"
    dat_var <- "daily_symptom_onset"
  }
  else if (type == "confirmation") {
    model_var <- "CR"
    dat_var <- "daily_confirmed"
  }
