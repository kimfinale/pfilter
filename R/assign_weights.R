...
#' @export
#' @examples
#'  wt <- assign_weights(predicted = latent_var, t = t, data = data, data_type = type)
assign_weights <- function (var,
                            t,
                            data,
                            data_type = c("infection", "symptom onset", "confirmation"),
                            error_pdf = c("pois", "negbin", "binom"),
                            negbin_size = 5, binom_prob = 0.8) {

  type <- match.arg(data_type)
  error_pdf <- match.arg(error_pdf)
  # select the correct data and model predicted values depending on the data type
  if (type == "infection") {
  ...
  if (!is.na(case_data)) {
    ...
    if (error_pdf == "pois") {
      log_lik <- dpois(round(case_data), lambda = expected_val,
                       log = T)
    }
    else if (error_pdf == "negbin") {
      log_lik <- dnbinom(round(case_data), size = negbin_size,
                         mu = expected_val, log = T)
    }
    else if (error_pdf == "binom") {
      log_lik <- dbinom(round(case_data), size = round(expected_val),
  ...
  return (exp(log_lik)) # convert to the normal probability
}
